...
public class AddressBook {
    ...
    public void searchContactByStateOrCity() {
        ...
        listObjects = cityContactMap.entrySet().stream().filter(e -> e.getKey().equals(cityName))
                .flatMap(e -> e.getValue().stream()).filter(e -> e.getFirstName().equals(firstName))
    ...
    public boolean duplicateEntriesCheck(String firstName) {
        ...
        dpNameCheck = mapper.entrySet().stream().map(e -> e.getValue().stream())
                .flatMap(e -> e.filter(s -> s.getFirstName().equals(firstName))).collect(Collectors.toList());
    ...
    public void add() throws IOException {
        ...
        for (int i = 0; i < NO_OF_CONTACTS; i++) {
            ...
            AddressBook fileObject = new AddressBook(last_Name, first_Name, address, cityName, stateName, phone_Number,
                    zip_Code, email);
            writeToFile(Address_Book_Name, fileObject);

            cityContactMap.computeIfAbsent(cityName, cityName -> new ArrayList<>()).add(new AddressBook(last_Name,
                    first_Name, address, cityName, stateName, phone_Number, zip_Code, email));
            writeToFile(cityName, new AddressBook(last_Name, first_Name, address, cityName, stateName, phone_Number,
                    zip_Code, email));
            stateContactMap.computeIfAbsent(stateName, stateName -> new ArrayList<>()).add(new AddressBook(last_Name,
                    first_Name, address, cityName, stateName, phone_Number, zip_Code, email));
    ...
    public void display() throws IOException {
        ...
        Path addressBookDirectory = Paths.get("E:\\Eclipse_Practise\\AddressBook\\CsvDataFiles");
        ...
        if (Files.isDirectory(addressBookDirectory)) {
            String newFile = addressBookDirectory + "/" + address_Book + ".csv";
            Path newFilePath = Paths.get(newFile);
            if (Files.exists(newFilePath)) {
                CSVReader reader = new CSVReader(new FileReader(String.valueOf(newFilePath)));
                String[] nextLine;
                while ((nextLine = reader.readNext()) != null) {
                    nextLine = reader.readNext();
                    System.out.print("First Name :" + nextLine[0]);
                    System.out.println("\n");
                    System.out.print("Last Name :" + nextLine[1]);
                    System.out.println("\n");
                    System.out.print("Phone :" + nextLine[2]);
                    System.out.println("\n");
                    System.out.print("Email " + nextLine[3]);
                    System.out.println("\n");
                    System.out.print("State :" + nextLine[4]);
                    System.out.println("\n");
                    System.out.print("Address :" + nextLine[5]);
                    System.out.println("\n");
                    System.out.print("Zip :" + nextLine[6]);
                    System.out.println("\n");
                    System.out.print("City :" + nextLine[7]);
                }
            }
        }
    }
    ...
    public void displayState() {
        ...
        String stateNameFilter = sc.next();
        List<Object> objectList = stateContactMap.entrySet().stream().filter(e -> e.getKey().equals(stateNameFilter))
                .collect(Collectors.toList());
        System.out.println("List of city objects :" + objectList.toString());
    }
    ...
    public void writeToFile(String AddressBookName, AddressBook fileObjectToBeWritten) throws IOException {
        Path directoryPath = Paths.get("E:\\Eclipse_Practise\\AddressBook\\CsvDataFiles");
        File directoryObject = directoryPath.toFile();
        if (Files.isDirectory(directoryPath)) {
            String fileName = directoryPath + "/" + AddressBookName + ".csv";
            Path filePath = Paths.get(fileName);
            File fileObject = filePath.toFile();
            if (!Files.exists(filePath)) {
                Files.createFile(filePath);
                BufferedWriter out = new BufferedWriter(new FileWriter(String.valueOf(filePath)));
                appendStrToFile(String.valueOf(filePath),
                        "First Name,Last Name, Phone, Email, State, Address, Zip, City");
                appendStrToFile(String.valueOf(filePath), "\n");
                appendStrToFile(String.valueOf(filePath), fileObjectToBeWritten.FIRST_NAME);
                appendStrToFile(String.valueOf(filePath), ",");
                appendStrToFile(String.valueOf(filePath), fileObjectToBeWritten.LAST_NAME);
                appendStrToFile(String.valueOf(filePath), ",");
                appendStrToFile(String.valueOf(filePath), fileObjectToBeWritten.PHONE_NUMBER);
                appendStrToFile(String.valueOf(filePath), ",");
                appendStrToFile(String.valueOf(filePath), fileObjectToBeWritten.EMAIL);
                appendStrToFile(String.valueOf(filePath), ",");
                appendStrToFile(String.valueOf(filePath), fileObjectToBeWritten.STATE);
                appendStrToFile(String.valueOf(filePath), ",");
                appendStrToFile(String.valueOf(filePath), fileObjectToBeWritten.ADDRESS);
                appendStrToFile(String.valueOf(filePath), ",");
                appendStrToFile(String.valueOf(filePath), fileObjectToBeWritten.ZIP_CODE);
                appendStrToFile(String.valueOf(filePath), ",");
                appendStrToFile(String.valueOf(filePath), fileObjectToBeWritten.CITY);
                appendStrToFile(String.valueOf(filePath), "\n");
